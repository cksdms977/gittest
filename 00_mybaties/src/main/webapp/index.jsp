...
<head>
<meta charset="UTF-8">
<title>기본 mybatis적용하기</title>
</head>
<body>
	<h2>mybatis 실행 해보기</h2>
	...
	<form action="${path }/deleteStudent.do" method="post">
		<input type="number" name="no" placeholder="번호"><br>
		<input type="submit" value="저장"><br>
	</form>
	
	<h2>DB에 저장된 데이터 조회하는 기능 구현하기</h2>
	<p>select문을 이용해서 DB에서 조회하고 그 결과를 java의 특정 클래스로 저장</p>
	<p>select문을 실행한 후 조회한 DB에 데이터와 지정한 java 클래스를 연결해줌</p>
	<h3>학생 수 조회하기 </h3> 
	...
	<h4><a href="${path }/student/select/selectStudentListMap">전체학생조회</a></h4>
