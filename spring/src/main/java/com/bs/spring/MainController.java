...
public class MainController {
	...
	public String main(HttpServletRequest req, HttpSession session, HttpServletResponse res) {
		...
		// log를 출력할때는 logger가 제공하는 메소드를 이용한다.
		//1. debug() : 개발시에 사용하는 로그를 출력할때 사용 
		//2. info() : 프로그램이 샐행하는 정보를 출력할때 사용 
		//3. warn() : 잘못된 사용을 했을때 출력할때 사용 
		//4. error(): Exception 이나 실행이 불가능한 기능에 대한 로그를 출력할때 사용
		// *메소드의 매개변수의 기본적으로 String만 가능 , 객체나 다른 데이터를 출력하려면? ("{}", 출력한변수)
		
		
		//level
		// debug -> info -> wqrn -> error -> fatal
		// 로그 출력하기
