...
<div id="enroll-container">
			<form name="memberEnrollFrm" action="${path }/member/insertMember.do" method="post" >
				...
				<input type="text" class="form-control" placeholder="주소" value="${member.address }" name="address" id="address">
				...
				<div class="form-check-inline form-check">
					취미 : &nbsp; 
					<input type="checkbox" class="form-check-input" name="hobby" id="hobby0" value="운동" ${member.hobby.stream().anyMatch(e->e=='운동').get()?"checked":"" }
					<%-- ${fn:join(member.hobby, ",").contains("운동")?"checked":"" } --%>
					><label for="hobby0" class="form-check-label">운동</label>&nbsp;
					<input type="checkbox" class="form-check-input" name="hobby" id="hobby1" value="등산" ${fn:join(member.hobby, ",").contains("등산")?"checked":"" }><label for="hobby1" class="form-check-label">등산</label>&nbsp;
					<input type="checkbox" class="form-check-input" name="hobby" id="hobby2" value="독서" ${fn:join(member.hobby, ",").contains("독서")?"checked":"" }><label for="hobby2" class="form-check-label">독서</label>&nbsp;
					<input type="checkbox" class="form-check-input" name="hobby" id="hobby3" value="게임" ${fn:join(member.hobby, ",").contains("게임")?"checked":"" }><label for="hobby3" class="form-check-label">게임</label>&nbsp;
					<input type="checkbox" class="form-check-input" name="hobby" id="hobby4" value="여행" ${fn:join(member.hobby, ",").contains("여행")?"checked":"" }><label for="hobby4" class="form-check-label">여행</label>&nbsp;
				</div>
				<br />
				<input type="submit" class="btn btn-outline-success" value="수정" >&nbsp;
				<button class="btn btn-outline-success">탈퇴</button>
			</form>
		</div>
